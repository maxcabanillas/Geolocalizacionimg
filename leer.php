<?php
	...
	if($idTipo==1){		
		...
		$query="select v.idvigencia, es.idEspectacular, es.idciudad, es.Clave, Latitud, Longitud, Direccion, Icono, w, h, Foto, es.Activo,
		...
		if($_GET['campanas']!="0"){
			...
			$conteo = count($arrayCampana);
				$queryCamp = "SELECT es.clave, v.idvigencia, esv.idespectaculares FROM campanas c join vigencia v on v.idcampana = c.idcampanas join
								espectaculares_vigencia esv on esv.idvigencia = v.idvigencia join
								espectaculares es on es.idespectacular = esv.idespectaculares
								where c.idcampanas in (".$Campanas.") and v.fecha_ini <= CURDATE() and v.fecha_fin >= CURDATE()";
			...
			for($a=0;$a<$conteo;$a++){
				$queryCamp = "SELECT v.idvigencia, esv.idespectaculares FROM campanas c join vigencia v on v.idcampana = c.idcampanas join
								espectaculares_vigencia esv on esv.idvigencia = v.idvigencia
								where  c.idcampanas = ".$arrayCampana[$a]." and v.fecha_ini <= CURDATE() and v.fecha_fin >= CURDATE() 
